...
public class AbHttpClient {
	...
	// private static final String AUTHACCOUNT = "admin:admin123456";// 测试
	private static final String AUTHACCOUNT = "admin:ruiXZTDN76";// 发布
	private static final String Authrization = "Basic "
			+ AbBase64.encode(AUTHACCOUNT.getBytes());// "Basic YWRtaW46YWRtaW4xMjM0NTY=";
	/** CookieStore. */
	...
	/** 缓冲大小. */
	private static final int DEFAULT_SOCKET_BUFFER_SIZE = 16 * 1024;
	 ...
	 * 
	 * @param url
	 *            the url 如果有特殊字符需要URLEncoder
	 * @param params
	 *            the params 不需要URLEncoder
	 * @param responseListener
	 *            the response listener
	 */
	public void getWithoutThread(String url, AbRequestParams params,
		...
		try {
			...
			// 取得HttpResponse
			httpClient.execute(httpGet, new RedirectionResponseHandler2(url,
					responseListener), mHttpContext);
		} catch (Exception e) {
			e.printStackTrace();
			// 发送失败消息
			responseListener.onFailure(AbHttpStatus.UNTREATED_CODE,
					e.getMessage(), new AbAppException(e));
		}

		// HTTP URL实现的，为了sessionId 保持相同client
		/*
		 * HttpURLConnection urlConn = null; InputStream is = null; try {
		 * responseListener.onStart();
		 * 
		 * if(!AbAppUtil.isNetworkAvailable(mContext)){ Thread.sleep(200);
		 * responseListener
		 * .onFailure(AbHttpStatus.CONNECT_FAILURE_CODE,AbAppConfig
		 * .CONNECT_EXCEPTION, new
		 * AbAppException(AbAppConfig.CONNECT_EXCEPTION)); return; }
		 * 
		 * String resultString = null;
		 * 
		 * if(params!=null){ if(url.indexOf("?")==-1){ url += "?"; } url +=
		 * params.getParamString(); } URL requestUrl = new URL(url); urlConn =
		 * (HttpURLConnection) requestUrl.openConnection();
		 * urlConn.setRequestMethod("GET"); urlConn.setConnectTimeout(mTimeout);
		 * urlConn.setReadTimeout(mTimeout); is = urlConn.getInputStream(); if
		 * (urlConn.getResponseCode() == HttpStatus.SC_OK){ resultString =
		 * readString(is); }else{ resultString =
		 * readString(urlConn.getErrorStream()); } is.close();
		 * responseListener.onSuccess(AbHttpStatus.SUCCESS_CODE, resultString);
		 * } catch (Exception e) { e.printStackTrace(); AbLogUtil.i(mContext,
		 * "[HTTP GET]:"+url+",error："+e.getMessage()); //发送失败消息
		 * responseListener
		 * .onFailure(AbHttpStatus.UNTREATED_CODE,e.getMessage(),new
		 * AbAppException(e)); } finally { if (urlConn != null){
		 * urlConn.disconnect(); } responseListener.onFinish(); }
		 */
	}
	 ...
	 *            the response listener
	 */
	public void postWithoutThread(String url, AbRequestParams params,
		...
		try {
			...
			if (!AbAppUtil.isNetworkAvailable(mContext)) {
				Thread.sleep(200);
				responseListener.onFailure(AbHttpStatus.CONNECT_FAILURE_CODE,
						AbAppConfig.CONNECT_EXCEPTION, new AbAppException(
								AbAppConfig.CONNECT_EXCEPTION));
				return;
			}
			...
			// 取得HttpResponse
			httpClient.execute(httpPost, new RedirectionResponseHandler2(url,
					responseListener), mHttpContext);

		} catch (Exception e) {
			e.printStackTrace();
			AbLogUtil.i(mContext,
		...
		/*
		 * HttpURLConnection urlConn = null; InputStream is = null; OutputStream
		 * os = null; try { responseListener.onStart();
		 * 
		 * if(!AbAppUtil.isNetworkAvailable(mContext)){ Thread.sleep(200);
		 * responseListener
		 * .onFailure(AbHttpStatus.CONNECT_FAILURE_CODE,AbAppConfig
		 ...
		 * AbAppException(AbAppConfig.CONNECT_EXCEPTION)); return; }
		 * 
		 * String resultString = null; URL requestUrl = new URL(url); urlConn =
		 * (HttpURLConnection) requestUrl.openConnection();
		 * urlConn.setRequestMethod("POST");
		 * urlConn.setConnectTimeout(mTimeout);
		 * urlConn.setReadTimeout(mTimeout); urlConn.setDoOutput(true); os =
		 * urlConn.getOutputStream();
		 * 
		 * //是否包含文件 boolean isContainFile = false;
		 * 
		 * if(params != null){ //使用NameValuePair来保存要传递的Post参数设置字符集 HttpEntity
		 * reqEntity = params.getEntity(); reqEntity.writeTo(os);
		 * if(params.getFileParams().size()>0){ isContainFile = true; } }
		 * 
		 * if(isContainFile){ urlConn.setRequestProperty("connection",
		 * "keep-alive"); urlConn.setRequestProperty("Content-Type",
		 ...
		 * os.flush(); os.close();
		 * 
		 * is = urlConn.getInputStream(); if (urlConn.getResponseCode() ==
		 * HttpStatus.SC_OK){ resultString = readString(is); }else{ resultString
		 * = readString(urlConn.getErrorStream()); } is.close();
		 * responseListener.onSuccess(AbHttpStatus.SUCCESS_CODE, resultString);
		 * } catch (Exception e) { e.printStackTrace(); AbLogUtil.i(mContext,
		 * "[HTTP POST]:"+url+",error："+e.getMessage()); //发送失败消息
		 * responseListener
		 * .onFailure(AbHttpStatus.UNTREATED_CODE,e.getMessage(),new
		 * AbAppException(e)); } finally { if (urlConn != null){
		 * urlConn.disconnect(); } responseListener.onFinish(); }
	 ...
	 *            the response listener
	 */
	public void get(final String url, final AbRequestParams params,
			final AbHttpResponseListener responseListener) {
	 ...
	 *            the response listener
	 */
	public void getWithCache(final String url, final AbRequestParams params,
			final AbHttpResponseListener responseListener) {
		responseListener.setHandler(new ResponderHandler(responseListener));
		responseListener.onStart();
		mExecutorService.execute(new Runnable() {
			public void run() {
				try {
					...
					if (entry == null || entry.isExpired()) {
						AbLogUtil.i(AbHttpClient.class, "磁盘中没有缓存，或者已经过期");
						...
						AbCacheResponse response = AbCacheUtil
								.getCacheResponse(httpUrl);

						if (response != null) {
							String responseBody = new String(response.data);
							AbLogUtil.i(mContext, "[HTTP GET]:" + httpUrl
									+ ",result：" + responseBody);
							Entry entryNew = AbCacheHeaderParser
									.parseCacheHeaders(response, cacheMaxAge);
							if (entryNew != null) {
								diskCache.put(cacheKey, entryNew);
								AbLogUtil.i(mContext, "HTTP 缓存成功");
							} else {
								AbLogUtil.i(AbHttpClient.class,
										"HTTP 缓存失败，parseCacheHeaders失败");
							}
							((AbStringHttpResponseListener) responseListener)
									.sendSuccessMessage(
											AbHttpStatus.SUCCESS_CODE,
											responseBody);
						} else {
							responseListener
									.sendFailureMessage(
											AbHttpStatus.SERVER_FAILURE_CODE,
											AbAppConfig.REMOTE_SERVICE_EXCEPTION,
											new AbAppException(
													AbAppConfig.REMOTE_SERVICE_EXCEPTION));
						}
					} else {
						...
						((AbStringHttpResponseListener) responseListener)
								.sendSuccessMessage(AbHttpStatus.SUCCESS_CODE,
										responseBody);
						AbLogUtil.i(mContext, "[HTTP GET CACHED]:" + httpUrl
								+ ",result：" + responseBody);
					}
					...
					// 发送失败消息
					responseListener.sendFailureMessage(
							AbHttpStatus.UNTREATED_CODE, e.getMessage(),
	 ...
	 *            the response listener
	 */
	private void doGet(String url, AbRequestParams params,
			AbHttpResponseListener responseListener) {
		try {
			...
			// "admin/123456".getBytes(), Base64.DEFAULT)));
			httpGet.addHeader("Authorization", Authrization);

			// 取得默认的HttpClient
			HttpClient httpClient = getHttpClient();
			// 取得HttpResponse
			httpClient.execute(httpGet, new RedirectionResponseHandler(url,
					responseListener), mHttpContext);
		} catch (Exception e) {
			e.printStackTrace();
			// 发送失败消息
			responseListener.sendFailureMessage(AbHttpStatus.UNTREATED_CODE,
					e.getMessage(), new AbAppException(e));
		}
	}
	 ...
	 *            the response listener
	 */
	private void doPost(String url, AbRequestParams params,
			AbHttpResponseListener responseListener) {
		try {
			...
			// + new String(Base64.encode(
			// "admin/123456".getBytes(), Base64.DEFAULT)));
			httpPost.addHeader("Authorization", Authrization);
			// 是否包含文件
			boolean isContainFile = false;
			if (params != null) {
				// 使用NameValuePair来保存要传递的Post参数设置字符集
			...
			// 取得HttpResponse
			httpClient.execute(httpPost, new RedirectionResponseHandler(url,
					responseListener), mHttpContext);
	 ...
	 * @return
	 */
	public void postJson(final String url, final AbJsonParams params,
			final AbStringHttpResponseListener responseListener) {
		responseListener.setHandler(new ResponderHandler(responseListener));
		responseListener.onStart();
		mExecutorService.execute(new Runnable() {
			public void run() {
				...
				try {
					...
					if (params != null) {
						...
						urlConn.setRequestProperty("Content-Type",
								"application/json");
						body = StringBody.create(params.getJson(),
					...
					if (body != null) {
						AbLogUtil.i(mContext, "[HTTP POST]:" + url + ",body:"
								+ params.getJson());
					} else {
					...
					responseListener.sendSuccessMessage(
							AbHttpStatus.SUCCESS_CODE, resultString);
				} catch (Exception e) {
					e.printStackTrace();
					AbLogUtil.i(mContext,
							"[HTTP POST]:" + url + ",error：" + e.getMessage());
	 ...
	 * @return
	 */
	public void doRequest(final String url, final AbRequestParams params,
			final AbStringHttpResponseListener responseListener) {
		responseListener.setHandler(new ResponderHandler(responseListener));
		responseListener.onStart();
		mExecutorService.execute(new Runnable() {
	...
	public void writeResponseData(Context context, HttpEntity entity,
			String name, AbFileHttpResponseListener responseListener) {
		...
		try {
			...
			// 发送失败消息
			responseListener.sendFailureMessage(
					AbHttpStatus.RESPONSE_TIMEOUT_CODE,
	 ...
	 *            the response listener
	 */
	public void writeResponseData2(Context context, HttpEntity entity,
			String name, AbFileHttpResponseListener responseListener) {
		...
		try {
			...
			e.printStackTrace();
			// 发送失败消息
			responseListener.onFailure(AbHttpStatus.RESPONSE_TIMEOUT_CODE,
					AbAppConfig.SOCKET_TIMEOUT_EXCEPTION, e);
		} finally {
			try {
				if (inStream != null) {
	 ...
	 *            the response listener
	 */
	public void readResponseData2(HttpEntity entity,
			AbBinaryHttpResponseListener responseListener) {
		...
		try {
			...
			responseListener
					.onSuccess(HttpStatus.SC_OK, outSteam.toByteArray());
		} catch (Exception e) {
			e.printStackTrace();
			// 发送失败消息
			responseListener.onFailure(AbHttpStatus.RESPONSE_TIMEOUT_CODE,
		...
		public void handleMessage(Message msg) {
				...
				if (response != null) {
					if (responseListener instanceof AbStringHttpResponseListener) {
						if (response.length >= 2) {
							...
							AbLogUtil.e(mContext, "SUCCESS_MESSAGE "
									+ AbAppConfig.MISSING_PARAMETERS);
						}
			...
			case PROGRESS_MESSAGE:
				...
				if (response != null && response.length >= 2) {
					...
					AbLogUtil.e(mContext, "PROGRESS_MESSAGE "
							+ AbAppConfig.MISSING_PARAMETERS);
				}
				break;
			case RETRY_MESSAGE:
		...
		public String handleResponse(HttpResponse response)
			...
			if (statusCode == HttpStatus.SC_OK) {
				...
				if (entity != null) {
					if (mResponseListener instanceof AbStringHttpResponseListener) {
						...
						if (header != null) {
							...
							if (contentEncoding != null) {
								if (contentEncoding.contains("gzip")) {
									entity = new AbGzipDecompressingEntity(
											entity);
								}
							}
						}
						...
						AbLogUtil.i(mContext,
								"[HTTP Response]:" + request.getURI()
										+ ",result：" + responseBody);

						((AbStringHttpResponseListener) mResponseListener)
						...
						readResponseData(
								entity,
								((AbBinaryHttpResponseListener) mResponseListener));
					} else if (mResponseListener instanceof AbFileHttpResponseListener) {
						// 获取文件名
						String fileName = AbFileUtil.getCacheFileNameFromUrl(
								mUrl, response);
						AbLogUtil.i(mContext,
								"[HTTP Response]:" + request.getURI()
										+ ",result：" + fileName);
						writeResponseData(
								...
								entity,
								fileName,
								((AbFileHttpResponseListener) mResponseListener));
					}
					// 资源释放!!!
					try {
			...
			// 301 302进行重定向请求
				...
				mResponseListener.sendFailureMessage(statusCode,
						AbAppConfig.NOT_FOUND_EXCEPTION, new AbAppException(
								AbAppConfig.NOT_FOUND_EXCEPTION));
				// 完成消息
				mResponseListener.sendFinishMessage();
			} else if (statusCode == HttpStatus.SC_FORBIDDEN) {
				// 403
				mResponseListener.sendFailureMessage(statusCode,
						AbAppConfig.FORBIDDEN_EXCEPTION, new AbAppException(
								AbAppConfig.FORBIDDEN_EXCEPTION));
				// 完成消息
				mResponseListener.sendFinishMessage();
			} else {
				mResponseListener
	 ...
	 * 使用ResponseHandler接口处理响应,支持重定向
	 */
	private class RedirectionResponseHandler2 implements
		...
		public String handleResponse(HttpResponse response)
			...
			// 301 302进行重定向请求
				...
				String location = locationHeader.getValue();
				if (request.getMethod().equalsIgnoreCase(HTTP_POST)) {
					postWithoutThread(location, new AbRequestParams(),
							mResponseListener);
				} else if (request.getMethod().equalsIgnoreCase(HTTP_GET)) {
					getWithoutThread(location, new AbRequestParams(),
							mResponseListener);
				}
			} else if (statusCode == HttpStatus.SC_NOT_FOUND) {
				// 404
				mResponseListener.onFailure(statusCode,
						AbAppConfig.NOT_FOUND_EXCEPTION, new AbAppException(
								AbAppConfig.NOT_FOUND_EXCEPTION));
				// 完成消息
				mResponseListener.onFinish();
